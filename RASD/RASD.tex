...
\begin{itemize}
    ...
    \begin{itemize}
    		\item {[G.6.1]}If allowed the Municiplaity, SafeStreets can cross their information about accidents with its own in order to improve the analysis.
    		\item \item {[G.6.2]}If the System collects multiple reports of the same violation in a given area, it can suggest possible interventions to the Authorities.
    \end{itemize}    
    \item{[G.7]} If allowed by the Municiplaity, SafeStreets can access their information about issued tickets to build statistics and evaluate its effectiveness.
    \begin{itemize}
...
a Citizien or an Authority.\\
\\ 
\textbf{Commit of a report} (world controlled, machine observed)\\
A Citizen can send a Report with a description of the violation compiling a structured field. He/she has the possibility to  attach a picture. The System will check if the compiled fields are consistent and complete, then it will encrypt the Report and send it to the DBMS (Figure \ref{fig:State1}). \\
\\
...
\begin{enumerate}
    ...
    \begin{itemize}
    	...
    	\item [R.20]
	\end{itemize} 
	\ {[G.5]} The System must allows Authorities to consult Reports. 
	\begin{itemize}
		\item [R.16] notification
		\item [R.17] show most recent reports (authority)
    	\item [R.18] 
    	\item [D.4] authority intervengono
	\end{itemize} 
    \item {[G.6]} The System analyzes the stored information to build statistics.
    \begin{itemize}
    	...
    	\item [D.7]
	\end{itemize}    
	\item {[G.7]} The System is able to detect unsafe areas.
	\begin{itemize}
