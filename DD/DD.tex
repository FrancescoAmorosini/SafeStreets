...
\item \textbf{Report Manager}: it's a \textit{subsystem} component in charge of all the processing functions on the newly received reports:
\begin{itemize}
    ...
    \item \textbf{Notify Controller}: the role of this component is to notify the correct authorities based on their municipality, using its \textit{AuthorityList}. It depends on the Report Controller, in the sense that it will notify the authorities only if the check of the report is successful and the type of the report is \textit{Traffic Violation}.
