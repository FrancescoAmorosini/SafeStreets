...
\end{figure}
Once a guest has undergone the registration procedure, he/she can log in to access the application's functionalities. To do that, he/she fills the login form with the credentials provided during the registration phase. The entered data are sent to the Application Server through an HTTP \texttt{POST}. The \textit{Authentication Controller} checks if the data provided are valid: it asks the \textit{Storage Interface} to run a \texttt{SELECT} query on the database to verify that the data provided belong to a registered user. If the query returns a non-null result, then the \textit{Authentication Controller} requests to login the guest and it returns a successmessage, otherwise an HTTP error is sent back to the guest which has not been able to log in successfully.
...
\end{figure}
A citizen is logged in and opens the menu to send a new report. The \textit{Report Interface} provides the report form that has to be filled. The \textit{Report Interface} also communicates with the \textit{Report Controller}, thus once the report is compiled and sent by the citizen, the \textit{Report Interface} sends it to the \textit{Report Controller} to check its validity. 
If the citizen did not insert the license plate in the report, the \textit{Report Controller} uses an external OCR service which executes an OCR algorithm to retrieve the license plate from the picture provided by the citizen.
At this point, when all the data are avaiable, the report is shared with the \textit{Notify Controller} which will check its authority list to notify the right authority.
...
Security Manager} uses the Storage Interface to retrieve all the reports needed and decrypts them.  After that, the decrypted data is sent the \textit{Statistic Controller} to compute the statistics on them. If the municipality has given the permission to access their database, the Storage Interface  can also retrieve them contacting the \textit{Municipality Interface}, and cross them with SafeStreets's.
...
\end{figure}
A logged in user wants to retrieve all the reports he/she has sent up to that moment with a \texttt{GET} request. The \textit{History Manager} handles this functionality communicating with the \textit{Storage Interface}, which is able to retrieve all the reports relative a specific user. This is done by running a \texttt{SELECT} query on the database and, if the query result is non-null, an HTTP Success message is returned to the user and the Storage Manager shows the reports; otherwise an HTTP Error is send back to the User.
In case the user is an authority, he/she can retrieve all the submitted reports in a given area or period of time.  
\clearpage
...
here we show an example of how data can be organised in a Entity Relation Schema.
\begin{figure}[!ht]
	\begin{center}
