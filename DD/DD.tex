...
\item \textbf{History Manager}: its function is to retrieve and show to the Client his personal report's history.
...
\begin{table}[!ht]
	\begin{longtable}{C{.30\textwidth}||L{.60\textwidth}}
		\toprule
		\textbf{Component(DD)} & \textbf{Requirement(RASD)}\\
		\midrule
		\midrule
		Auth Controller & $\bullet$ {[R.1]} A visitor must be able to register. During the registration the System will ask to provide some credentials. \\
		...
		& $\bullet$ {[R.13]} Provide a recognition system of badge number (this may involve communication with the Municipality).
		\\
		\midrule
		
		User Controller &  $\bullet$ {[R.5]} Allow to change username, only if the new username is not already in use by another User, email, only if the new email is in a correct format and password, only if the new password is different from the precedent and respects the minimum length. \\
		...
		\midrule
		Report Interface & $\bullet$ {[R.8]} The Citizen must be allowed to create reports \\

		& $\bullet$ {[R.9]} Check if the Report created by the Citizen is valid. \\

		\midrule
		Notify Controller & $\bullet$ {[R.10]} Notify the Authorities whenever a new Report involving their municipality is submitted. \\

		& $\bullet$ {[R.16]} All traffic violations must be notified to Authorities according to their municipality. \\

		\midrule
		StatisticsSuggestions Interface & $\bullet$ {[R.11]} Provide two different level of visibility regarding the statistics. Only the Authorities can consult statistics with private data. \\

		& $\bullet$ {[R.12]} Safestreets' application must show the most recent version of the suggestions and statistics. \\
		
		\midrule
		Map Service Interface & {[R.15]} Allow Users to see the their position in a map.\\
	\end{longtable}
\end{table}
\begin{table}[!ht]
	\begin{longtable}{C{.30\textwidth}||L{.60\textwidth}}
		\midrule
		MISSING COMPONENT & $\bullet$ {[R.17]} Provide the Authorities with a list of the most recent reports submitted. \\

		& $\bullet$ {[R.18]} Allow Authorities to also consult older Reports. \\
		
		& $\bullet$ {[R.20]} Use third party services to enable some functions.\\

		& $\bullet$ {[R.21]} Statistics and suggestions have to be periodically recomputed.\\

		\midrule
		Storage Interface & $\bullet$ {[R.19]} Interact with the Municipality's System to get the permission for using their data.\\

		& $\bullet$ {[R.24]} Cross the data coming from external DBMSs with the System's own data.\\

		\midrule
		Security Manager & $\bullet$ {[R.22]} Provide protection of data using encryption.\\

		& $\bullet$ {[R.28]} After its validation, no one should be able to modify a submitted Report.\\

		\midrule
		Statistic Controller & $\bullet$ {[R.23]} Acknowledge whether or not multiple istances of Reports are referred to the same infraction, even if protracted over time.\\

		& $\bullet$ {[R.25]} Cross data from different Reports in order to establish their truthfulness.\\

		\midrule
		AI Controller & $\bullet$ {[R.26]} Unsafe areas are detected by means of an artificial intelligence.\\
...
\end{itemize}
%The following table lists the main features avaiable, their importance and %implementation difficulty is shown below to better understand the decision about %implementation, testing and integration that will be taken in the rest of this section.
